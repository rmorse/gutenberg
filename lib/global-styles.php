...
function gutenberg_experimental_global_styles_get_stylesheet( $tree ) {
	...
	foreach ( array_keys( $tree ) as $block_name ) {
		...
		foreach ( $presets_structure as $preset_structure ) {
			$block_preset = gutenberg_experimental_get( $tree[ $block_name ]['settings'], $preset_structure['path'] );
			if ( ! empty( $block_preset ) ) {
				$computed_presets[ $preset_structure['token'] ] = array();
				foreach ( $block_preset as $preset_value ) {
					$computed_presets[ $preset_structure['token'] ][ $preset_value['slug'] ] = $preset_value['value'];
				}
			}
		}
